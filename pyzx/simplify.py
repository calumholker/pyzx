...
def simp(
    ...
    :param match_filter: An optional filtering function on candidate vertices or edges, passed as a second argument to the match function. 
    ...
    while True:
        if match_filter:
            if num: 
                matches = match(g, match_filter, num = -1 if num == -1 else num-total_rewrites)
            else: 
                matches = match(g, match_filter)
        elif num: 
...
def selective_simp(
    ...
    **kwargs: Any
    ...
    :param g: The graph to be simplified
    :param get_matches: A match function which takes ``g``, ``match_filter`` and any ``**kwargs`` as inputs, 
    ...
    :param match_filter: An optional filtering function for candidate vertices or edges. 
    ...
    for match in get_matches(g, match_filter, **kwargs):
        score = match_score(g, match, **kwargs)
        if score is None: continue
        matches[match] = score
    unchecked_matches = matches.copy()
    num_rewrites = 0
    while unchecked_matches and num_rewrites != num:
        match = max(unchecked_matches, key=unchecked_matches.__getitem__)
        check_g = g.clone()
        apply_rule(check_g, rewrite, [match])
        if condition(check_g, match):
            num_rewrites += 1
            updated_matches = update_matches(g, check_g, matches, get_matches, match_score, match_filter, **kwargs)
            g.replace(check_g)
            matches = updated_matches.copy()
            unchecked_matches = updated_matches.copy()
            continue
        del unchecked_matches[match]
    return num_rewrites
...
def flow_2Q_simp(
    ...
    :param match_filter: An optional filtering function for candidate vertices and edges.
                        If provided should return False if a candidate should not be considered for a match. Defaults to None.
...
def update_2Q_simp_matches(
    ...
    **kwargs: Any
    ...
    update_match_filter = lambda candidate: candidate in matches_to_update and (not match_filter or match_filter(candidate))
    new_matches = get_matches(g_after, update_match_filter, **kwargs)
    ...
    for m in new_matches:
        m_score = match_score(g_after, m, **kwargs)
