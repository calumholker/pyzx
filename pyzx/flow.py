...
def gflow(g: BaseGraph[VT, ET]) -> Optional[Tuple[Dict[VT, int], Dict[VT, Set[VT]], int]]:
	...
	while True:
		...
		if not correct:
			if not candidates: return l, gflow, k
			return None
		else:
			processed.update(correct)
			k += 1

def cflow(g: BaseGraph[VT, ET]) -> Optional[Tuple[Dict[VT,int], Dict[VT,VT], int]]:
...
def cflow_plus(g: BaseGraph[VT, ET]) -> Optional[Tuple[Dict[VT,VT], Dict[VT,VT], Dict[VT,int], Dict[Tuple[VT,VT],int]]]:
		"""Compute the causal flow of a diagram in graph-like form.
		...
		if g_without_gadgets.num_edges() > (k*n - comb(k+1,2)): return None
...
def find_gadgets(g:BaseGraph[VT,ET]) -> Tuple[Dict[VT,VT],Dict[VT,FrozenSet[VT]]]:
	gadgets = dict()
...
def ComputeSuprema(g:BaseGraph[VT, ET], f:Dict[VT,VT], P:Dict[VT,VT], L:Dict[VT,int]) -> Optional[Dict[Tuple[VT,VT],int]]:
...
def InitStatus(g:BaseGraph[VT, ET], P:Dict[VT,VT], L:Dict[VT,int]) -> Tuple[Dict[Tuple[VT,VT],int],Dict[VT,Optional[Union[bool,str]]]]:
...
def TraverseInflWalk(g:BaseGraph[VT, ET], f:Dict[VT,VT], sup:Dict[Tuple[VT,VT],int], status:Dict[VT,Optional[Union[bool,str]]], v:VT) -> Tuple[Dict[Tuple[VT,VT],int],Dict[VT,Optional[Union[bool,str]]]]:
