...
class BaseGraph(Generic[VT, ET], metaclass=DocstringMeta):
    ...
    def copy(self, adjoint: bool = False, backend: Optional[str] = None) -> 'BaseGraph':
        """Returns a copy of the graph. If ``adjoint`` is set, 
    ...
    def compose(self, other: 'BaseGraph') -> None:
        ...
        for v in outputs: self.remove_vertex(v)
    ...
    def tensor(self, other: 'BaseGraph') -> 'BaseGraph':
        ...
        for v in inputs: g.set_row(v, minr)

        maxr = max((g.row(v) for v in outputs), default=1)
        for v in outputs: g.set_row(v, maxr)
    ...
    def apply_state(self, state: str) -> None:
        ...
        for i,s in enumerate(state):
            ...
            if s in ('0', '1'):
                ...
                if s == '-':
                    self.set_phase(v, Fraction(1))
            else: raise TypeError("Unknown input state " + s)
    ...
    def apply_effect(self, effect: str) -> None:
        ...
        for i,s in enumerate(effect):
            ...
            if s in ('0', '1'):
                ...
                if s == '-':
                    self.set_phase(v, Fraction(1))
            else: raise TypeError("Unknown output effect " + s)
    ...
    def auto_detect_io(self):
        ...
        for v in self.vertices():
            ...
            if self.vertex_degree(v) != 1: raise TypeError("Invalid ZX-diagram: Boundary-type vertex does not have unique neighbor")
            w = list(self.neighbors(v))[0]
            if self.row(w) > self.row(v):
                inputs.append(v)
            elif self.row(w) < self.row(v):
                outputs.append(v)
            else: raise TypeError("Boundary-type vertex at same horizontal position as neighbor. Can't determine whether it is an input or output.")
    ...
    def add_vertex(self, 
        ...
        if ground: self.set_ground(v, True)
        return v
    ...
    def check_two_pauli_phases(self, v1: VT, v1p: FractionLike, v2: VT, v2p: FractionLike) -> Optional[List[Optional[FractionLike]]]:
        """Used for post phase teleportation simplifications.
        Checks whether both ``v1`` and ``v2`` can have their phases fixed to a Pauli phase (e.g. for pivoting)
    ...
    def remove_isolated_vertices(self) -> None:
        ...
        for v in self.vertices():
            ...
            if d == 0:
                rem.append(v)
                ty = self.type(v)
                if ty == VertexType.BOUNDARY: raise TypeError("Diagram is not a well-typed ZX-diagram: contains isolated boundary vertex.")
                elif ty == VertexType.H_BOX: self.scalar.add_phase(self.phase(v))
            ...
            if d == 1: # It has a unique neighbor
                ...
                if t1 == t2:
                    if et == EdgeType.SIMPLE: self.scalar.add_node(self.phase(v) + self.phase(w))
                    else: self.scalar.add_spider_pair(self.phase(v), self.phase(w))
                else:
                    if et == EdgeType.SIMPLE: self.scalar.add_spider_pair(self.phase(v), self.phase(w))
                    else: self.scalar.add_node(self.phase(v) + self.phase(w))
